...
export default function ExpenseDetailPage() {
  ...
  const fetchExpenseDetail = async () => {
    try {
      ...
      if (snapshot.exists()) {
        ...
        const expenseItem: ExpenseItem = { key: params.id as string, ...data };
  ...
  const canAccess = userData && (userData.userType === "Accounts" || userData.userType === "GB");
      ...
      <ProtectedRoute>
        <div className="min-h-screen bg-gray-50 py-8 px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <button onClick={() => router.push("/dashboard")} className="mt-4 text-blue-600 hover:text-blue-800">← Back to Dashboard</button>
          </div>
        </div>
      </ProtectedRoute>
    );
  ...
  if (!expense) {
    return (
      <ProtectedRoute>
        <div className="min-h-screen bg-gray-50 py-8 px-4">
          <div className="max-w-4xl mx-auto">
            <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded-md">Expense record not found.</div>
            <button onClick={() => router.push("/expense-list")} className="mt-4 text-blue-600 hover:text-blue-800">← Back to Expense List</button>
          </div>
        </div>
      </ProtectedRoute>
    );
      ...
      <div className="min-h-screen bg-gray-50 py-8 px-4">
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center mb-6">
            <button onClick={() => router.push("/expense-list")} className="mr-4 text-blue-600 hover:text-blue-800">← Back to Expense List</button>
            <h1 className="text-3xl font-bold">{isEditing ? "Edit Expense" : "Expense Details"}</h1>
          </div>
                  ...
                  <div className="flex space-x-6">
                    {(["Cash", "Cheque", "NEFT"] as ModeOfPayment[]).map((mop) => (
                      <label key={mop} className="flex items-center">
                        <input type="radio" name="modeOfPayment" value={mop} checked={modeOfPayment === mop} onChange={(e) => setModeOfPayment(e.target.value as ModeOfPayment)} className="h-4 w-4 text-blue-600 focus:ring-blue-500" />
            ...
            <>
              ...
              <div className="bg-white rounded-lg shadow overflow-hidden">
                <div className="bg-red-500 text-white p-6">
                  ...
                  <p className="text-red-100 mt-1">{new Date(expense.date).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' })}{expense.billNumber && <span className="ml-4">Bill: {expense.billNumber}</span>}</p>
                </div>
                <div className="p-6 space-y-6">
                  <div><h3 className="text-lg font-semibold text-gray-800 mb-3 border-b pb-2">Payee Information</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div><p className="text-sm text-gray-500">Name</p><p className="text-base font-medium text-gray-900">{expense.name}</p></div>
                      <div><p className="text-sm text-gray-500">PAN Number</p><p className="text-base font-medium text-gray-900">{expense.panNumber || '-'}</p></div>
                    </div>
                  </div>
                  <div><h3 className="text-lg font-semibold text-gray-800 mb-3 border-b pb-2">Payment Details</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div><p className="text-sm text-gray-500">Category</p><p className="text-base font-medium text-gray-900">{expense.category}</p></div>
                      <div><p className="text-sm text-gray-500">Amount</p><p className="text-2xl font-bold text-red-600">₹ {expense.amount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p></div>
                      <div><p className="text-sm text-gray-500">Mode of Payment</p><p className="text-base font-medium text-gray-900">{expense.modeOfPayment || 'Cash'}</p></div>
                      {expense.chequeNumber && <div><p className="text-sm text-gray-500">{expense.modeOfPayment === 'Cheque' ? 'Cheque' : 'Reference'} Number</p><p className="text-base font-medium text-gray-900">{expense.chequeNumber}</p></div>}
                    </div>
                  </div>
                  <div><h3 className="text-lg font-semibold text-gray-800 mb-3 border-b pb-2">Record Information</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div><p className="text-sm text-gray-500">Input By</p><p className="text-base font-medium text-gray-900">{expense.inputBy || '-'}</p></div>
          ...
          {showDeleteConfirm && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                ...
                <p className="text-gray-600 mb-2">Are you sure you want to delete this expense record?</p>
                <p className="text-sm text-red-600 mb-6">Name: <strong>{expense.name}</strong> — ₹ {expense.amount.toLocaleString('en-IN')}<br />This action will also adjust the total expense and cannot be undone.</p>
                <div className="flex gap-3">
