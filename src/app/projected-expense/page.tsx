...
import { dbPath, ROUTES, hasAccess, getCurrentYearString, EXPENSE_CATEGORIES } from "../../utils/constants";
...
export default function ProjectedExpensePage() {
  const { user } = useAuth();
  ...
  const loadData = async () => {
    try {
      ...
      const projectedRef = ref(db, dbPath.projectedExpense(currentYear));
      ...
      // Compute actuals from Expense records
      const expenseRef = ref(db, dbPath.expense(currentYear));
      const expenseSnapshot = await get(expenseRef);

      const actualByCategory: Record<string, number> = {};
      EXPENSE_CATEGORIES.forEach((cat) => { actualByCategory[cat.value] = 0; });

      if (expenseSnapshot.exists()) {
        const expenseData = expenseSnapshot.val();
        Object.values(expenseData).forEach((record: any) => {
      ...
      console.error("Error loading projected expense data:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("Error saving projected expense:", error);
      alert("Error saving projected expense. Please try again.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="min-h-screen bg-gray-50 py-8 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center mb-6">
            ...
            <h1 className="text-3xl font-bold text-gray-800">Projected Expense</h1>
          </div>

          {/* Description */}
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
            <p className="text-red-800 text-sm">
              Set projected (budgeted) expense amounts for each category. Actual figures are automatically computed from recorded expense entries.
            </p>
          </div>
          ...
          {showSuccess && (
            <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-md mb-6">
              Projected expense saved successfully!
            </div>
          )}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {EXPENSE_CATEGORIES.map((cat) => {
                    ...
                    // For expenses: positive variance (projected > actual) is good (under budget)
                    // negative variance (actual > projected) is bad (over budget)
                    const isOverBudget = actualVal > projectedVal && projectedVal > 0;
                    const isUnderBudget = actualVal <= projectedVal && projectedVal > 0;
                              ...
                              <div className="w-16 bg-gray-200 rounded-full h-2">
                                <div
                                  className={`h-2 rounded-full ${
                                    percentage <= 100 ? 'bg-green-500' : percentage <= 125 ? 'bg-yellow-500' : 'bg-red-500'
                                  }`}
                                  style={{ width: `${Math.min(percentage, 100)}%` }}
                                />
                              </div>
                              <span className={`text-xs font-medium ${
                                percentage <= 100 ? 'text-green-600' : percentage <= 125 ? 'text-yellow-600' : 'text-red-600'
                              }`}>
                                {percentage}%
                              </span>
                            </div>
                ...
                <tfoot className="bg-gray-50">
                  <tr>
                    ...
                    <td className={`px-6 py-4 whitespace-nowrap text-right text-sm font-bold ${
                      totalProjected - totalActual >= 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
          ...
          <div className="mt-6 flex justify-end">
            <button
              onClick={handleSave}
              disabled={saving}
              className="bg-red-600 text-white px-8 py-3 rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 font-medium disabled:opacity-50"
            >
              {saving ? "Saving..." : "Save Projected Expense"}
            </button>
          </div>
        </div>
      </div>
